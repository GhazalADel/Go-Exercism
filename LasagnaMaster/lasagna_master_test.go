...
func TestAddSecretIngredient(t *testing.T) {
	tests := []secretTest{
		secretTest{
			...
			myList:      []string{"sauce", "noodles", "meat", "?"},
			expected:    []string{"sauce", "noodles", "meat", "marjoram"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(tt.myList, tt.expected) {
				t.Errorf("addSecretIngredient(%v, %v) = %v want %v", tt.friendsList, tt.myList, tt.myList, tt.expected)
			}
